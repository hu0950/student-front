...
KISSY.add('mod/charts', function(S, Core) {
    
    var CHART_ATTR = 'charts-holder',
        ...
        Charts = {
            ...
            client: function(param){
                ...
                if(this.hasCharts(param)) {
                    S.log('改节点已存在图表，请检查！', 'warn', 'mod/charts');
                    return null;
                }
                DOM.data(get(param.renderTo), CHART_ATTR, new Core(param));
                // return new Charts(param);
            },
            /**
             * 判断当前图表是否加载过
             * @param  {[type]}  param [description]
             * @return {Boolean}       [加载过返回 true; 未加载过返回 false]
             */
            hasCharts: function(param){
                return (DOM.data(get(param.renderTo), CHART_ATTR))? true : false;
            }
        };
    if(!PW.mod || !PW.mod.Charts){
        PW.namespace('mod.Charts');
        PW.mod.Charts = Charts;
    }
    return Charts;
}, {
    requires: [
