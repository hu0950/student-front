...
* ==NOTES:=============================================
* v3.0.0(2014-12-24):
*     采用原kissy提供的calendar组件，功能健全
*     refer:
*     http://docs.kissyui.com/docs/html/api/component/calendar
*
* v3.0.1(2015.9.16):by:cuiy
*     将加载日历样式提前，防止多次加载造成浪费
* ---------------------------------------------------------------------------*/
KISSY.add('mod/calendar', function(S, Client){
    var
    ...
    var 
        Calendar = {
            ...
            refresh: function(){
                ...
                S.each(query(el.calendar), function(c){
                    var
                        ...
                        sd = parseInt(new Date(DOM.attr(c, 'data-selected-date'))) || S.now();
                    DOM.data(c, SCROLL_KEY, that.client({
                        renderTo: c,
                        select:{
                            showTime: true,
                            dateFmt: fmt,
                            selected: DOM.attr(c, 'data-selected-date')
                        },
                        style:{
             ...
             */
            reload: function(trigger, param){
                var
                    that = this,
                    ksCal = DOM.data(trigger, SCROLL_KEY);
                ksCal.refresh(param);
            },
            //判断节点上是否有日历
             ...
             * @return {[type]} [description]
             */
            _buildParam: function(trigger, param){
                var
                    type = DOM.attr(trigger, 'data-type'),
                    fmt = DOM.attr(trigger, 'data-format'),
                    sd = parseInt(new Date(DOM.attr(trigger, 'data-selected-date'))) || S.now(),
                    opts = {
                        select:{
                            showTime: true,
                            dateFmt: fmt,
                            selected: DOM.attr(trigger, 'data-selected-date')
                        },
                        style:{
                            startDay: new Date(sd)
                        }
                    };
                return S.mix(opts, param, true, undefined, true);
            }
        }
     //初始化
    S.ready(function(){
    ...
    S.augment(Client, S.EventTarget, {
        ...
        _buildParam: function(param){
            var
                ...
                opts = {};
            //合并参数
            S.mix(opts, CONFIG, true, undefined, true);
            S.mix(opts, CAL_SETTTINGS, true, undefined, true);
    ...
    S.augment(Calendar, {
        ...
        Page:function (config, father) {
            ...
            this.html = [
                ...
                '<div class="ks-multi-select {$multiSelect}"><button class="ks-multi-select-btn btn btn-primary">确定</button></div>',
            ...
            this.nav_html = [
                ...
                '</p>',
                '<p>',
                '<button class="btn btn-primaryok">确定</button><button class="cancel">取消</button>',
                '</p>'
            ].join("");
...
KISSY.add('kscal/time',function(S, Node,Calendar) {
    ...
    S.augment(Calendar, {
        ...
        TimeSelector:function(ft, father) {
            ...
            this.button = Node('<button class="ct-ok btn btn-primary">确定</button>');
            //小时
